...
$mail->AltBody = 'Some error in sending password, send another request for registration';
	...
	if($mail->send()) {	
			mysqli_query($connection,"UPDATE student_registration SET Password = ".$rand." WHERE Serial_No='".$Serial_No."'");
			mysqli_close($connection);
			echo '<script>window.alert("Password sent to the Student\'s Email !")</script>';
	}
	else if(!$mail->send()){
